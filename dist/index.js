 ...
 */
export async function createConfig({ tsconfigRootDir = '.', browser = false, react = false, svelte = false, project = ['./tsconfig.json'], ecmaVersion = 2024, ...restParserOptions } = {}) {
    if (svelte && react) {
        throw new Error('Cannot use both react and svelte for the same configuration');
    }
    const configNode = !svelte && !react && !browser
        ? [
    ...
    const configPrettier = svelte
        ? [
            ...(await import('eslint-plugin-svelte')).configs['flat/recommended'],
            eslintPluginPrettierRecommended,
            eslintConfigPrettier,
            ...(await import('eslint-plugin-svelte')).configs['flat/prettier'],
            {
                files: ['**/*.svelte'],
                languageOptions: {
                    parserOptions: {
